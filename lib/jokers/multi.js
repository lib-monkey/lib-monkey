...
export class TypedJocker extends Joker {
  ...
  getValue() {
    ...
    let instance = new Type(...params.map(param => param instanceof Joker ? param.value : param));
    ...
    if(init && typeof init === 'function'){ // TODO: Add async Init option
      ...
      let res = init(instance, ...initWith.map(param => param instanceof Joker ? param.value : param));

      return res instanceof Type ? res : instance;
    } else {
      return instance;      
    }
  }
